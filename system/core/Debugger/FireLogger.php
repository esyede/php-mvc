...
class FireLogger implements LoggerInterface
{
    ...
    public function log($message, $priority = self::DEBUG)
    {
        ...
        || $args[0] instanceof \Throwable)) {
            ...
            && 'Debugger\Debugger' === $trace[0]['class']
            ...
            && 'Debugger\Debugger' === $trace[1]['class']
    ...
    private function jsonDump(&$var, $level = 0)
    {
        if (is_bool($var) || is_null($var) || is_int($var) || is_float($var)) {
            ...
            return Dumper::encodeString($var, $this->max_length);
        } elseif (is_array($var)) {
            static $marker;
            ...
            } elseif ($level < $this->max_depth || ! $this->max_depth) {
                $var[$marker] = true;
                $res = [];
