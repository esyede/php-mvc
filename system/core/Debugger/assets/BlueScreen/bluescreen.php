...
<div id="debugger-bs">
	...
	<div>
		...
		<?php do { ?>
			...
			<?php if ($level++): ?>
			<div class="panel"<?php if ($level === 2) echo ' id="debuggerCaused"' ?>>
			...
			<div class="panel">
			<h2><a data-debugger-ref="^+" class="debugger-toggle<?php echo ($collapsed = $expanded !== null) ? ' debugger-collapsed' : '' ?>">Source file</a></h2>
			...
			<?php if (isset($stack[0]['class']) && $stack[0]['class'] === 'Debugger\Debugger' && ($stack[0]['function'] === 'shutdownHandler' || $stack[0]['function'] === 'errorHandler')) unset($stack[0]) ?>
			<?php if ($stack): ?>
			<div class="panel">
				...
				<ol>
					...
					<?php if (isset($row['file']) && is_file($row['file'])): ?><a data-debugger-ref="^p + .file" class="debugger-toggle<?php if ($expanded !== $key) echo ' debugger-collapsed' ?>">source</a>&nbsp; <?php endif ?>

					<?php
						if (isset($row['object'])) echo "<a data-debugger-ref='^p + .object' class='debugger-toggle debugger-collapsed'>";
					...
					<?php if (isset($row['file']) && is_file($row['file'])): ?>
						<div class="<?php if ($expanded !== $key) echo 'debugger-collapsed ' ?>file"><?php echo self::highlightFile($row['file'], $row['line']) ?></div>
					<?php endif ?>
